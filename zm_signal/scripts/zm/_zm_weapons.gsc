	...
	for ( ;; )
	{
		...
		else // If the player HAS the weapon, check for ammo update
		{
			...
			else if ( player zm_score::can_player_purchase( ammo_cost ) ) // if the player does have this then give him ammo.
			{
				...
				if ( ammo_given )
				{
					player notify( "weapon_ammo_restocked", weapon );	
...
function weapon_give( weapon, is_upgrade = false, magic_box = false, nosound = false, b_switch_weapon = true ) // is_upgrade and magic_box are ignored
{
	...
	if(isdefined(level.weaponZMRayzorbackGun) && isdefined(level.weaponZMRayzorbackGunUpgraded))
	{
		if(self hasperk("specialty_additionalprimaryweapon") && (self HasWeapon( level.weaponZMRayzorbackGun ) || self HasWeapon( level.weaponZMRayzorbackGunUpgraded )))
			weapon_limit +=1;
	}
